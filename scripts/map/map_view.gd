...
func _build_world_points():
	...
	if map_config.has("world") and map_config.world.has("points"):
		for p in map_config.world.points:
			...
			var connect_func: Callable
			
			match t:
				"base":
					btn_color = Color(0.3, 0.5, 0.9, 0.9)  # 蓝色 - 基地
					border_color = Color(0.1, 0.3, 0.8, 1.0)  # 深蓝色边框
					connect_func = _on_base_pressed
				"explore":
					btn_color = Color(0.8, 0.3, 0.3, 0.9)  # 红色 - 探索区域
					border_color = Color(0.6, 0.1, 0.1, 1.0)  # 深红色边框
			...
			hover_style.shadow_color = Color(0, 0, 0, 0.4)  # 悬停时阴影加深
			btn.add_theme_stylebox_override("hover", hover_style)
			
			# 按下效果 - 边框变深，背景变暗
			var pressed_style = btn_style.duplicate()
			pressed_style.bg_color = pressed_style.bg_color.darkened(0.2)
			pressed_style.border_color = pressed_style.border_color.darkened(0.2)
			pressed_style.shadow_size = 2  # 按下时阴影变小
			...
			# 连接信号
			btn.pressed.connect(connect_func.bind(id))
			canvas.add_child(btn)
			point_nodes.append({"node": btn, "pos": pos, "id": id, "type": t})
...
func _on_explore_point_pressed(explore_id: String):
	...
	# 获取类型并显示
	var point_type = explore_data.get("type", "explore")
	var type_text = ""
	if point_type == "explore":

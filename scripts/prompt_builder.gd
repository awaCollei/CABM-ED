...
func build_system_prompt(trigger_mode: String = "passive", keep_long_term_memory_placeholder: bool = false) -> String:
	...
	# 获取物品提示词
	var item_prompt = _get_item_prompt()
...
func get_memory_context() -> String:
	...
	for memory in memories:
		# 提示词里添加“相对时间前缀”，避免在保存阶段写入绝对时间
		if memory.has("timestamp") and not str(memory.timestamp).is_empty():
			context_lines.append("%s %s" % [TimeUtil.to_relative_time_prefix(memory.timestamp), memory.content])
		else:
...
func _load_response_mode() -> String:
	...
	if config_mgr and config_mgr.has_method("load_response_mode"):
		return config_mgr.load_response_mode()
	return "verbal" # 默认为语言表达模式

func _get_item_prompt() -> String:
	"""获取物品提示词（如果有上传物品）"""
	# 检查是否有待处理的物品
	if not has_meta("pending_item"):
		return ""
	
	var item_data = get_meta("pending_item")
	...
	# 构建item字段的选项文本
	var options = "0=拒绝，1=收起来"
	
	# 只有食品或消耗品才有"立即使用"选项
	if item_type == "食品" or item_type == "消耗品":
		options += "，2=立即使用"

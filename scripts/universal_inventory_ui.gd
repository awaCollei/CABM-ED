...
var player_empty_button: Button
var container_panel: PanelContainer
var container_grid: GridContainer
var container_title: Label
var container_sort_button: Button
var container_empty_button: Button
...
func _ready():
	...
	if player_sort_button:
		player_sort_button.pressed.connect(_on_player_sort_pressed)
	if container_sort_button:
		container_sort_button.pressed.connect(_on_container_sort_pressed)
...
func _get_node_references():
	...
	player_title = get_node_or_null("Panel/HBoxContainer/PlayerPanel/VBox/Header/Title")
	player_sort_button = get_node_or_null("Panel/HBoxContainer/PlayerPanel/VBox/Header/SortButton")
	player_empty_button = get_node_or_null("Panel/HBoxContainer/PlayerPanel/VBox/Header/empty")
	...
	container_title = get_node_or_null("Panel/HBoxContainer/ContainerPanel/VBox/Header/Title")
	container_sort_button = get_node_or_null("Panel/HBoxContainer/ContainerPanel/VBox/Header/SortButton")
	container_empty_button = get_node_or_null("Panel/HBoxContainer/ContainerPanel/VBox/Header/empty")
	...
	info_icon = get_node_or_null("Panel/HBoxContainer/InfoPanel/VBox/IconContainer/ItemIcon")
...
func open_inventory_only():
	...
	# 仅背包模式，默认显示整理按钮
	set_sort_buttons_visible(true, false)
...
func set_sort_buttons_visible(player_visible: bool, container_visible: bool):
	"""设置整理按钮是否可见"""
	if player_sort_button:
		player_sort_button.visible = player_visible
	if player_empty_button:
		player_empty_button.visible = player_visible
		
	if container_sort_button:
		container_sort_button.visible = container_visible
...
func _show_item_info(item_data: Dictionary):
	...
	# 更新按钮状态
	var can_use = not is_unknown and not selected_is_weapon_slot and selected_storage_type == "player" and selected_slot_index != -1 and ((item_config.get("type") == "药品" and _is_in_explore_scene()) or item_config.get("type") == "食品")
...
func _on_use_button_pressed():
	...
	if item_type == "药品":
		...
		var use_messages = cfg.get("use", [])
		if use_messages is Array and not use_messages.is_empty():
			var msg = use_messages.pick_random()
			MessageDisplay.show_info_message(msg)
	else:
		return
	# 消耗一个
	if int(item.count) > 1:
...
func _on_player_sort_pressed():
	"""整理玩家背包"""
	if player_container:
		player_container.sort()
		_clear_selection()
		_clear_item_info()

func _on_container_sort_pressed():
	"""整理其他容器"""
	if other_container:
		other_container.sort()
		_clear_selection()
		_clear_item_info()

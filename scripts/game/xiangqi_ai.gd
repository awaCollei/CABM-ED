...
func get_next_move(board: Array, ai_side: int, difficulty: int) -> Dictionary:
	...
	match difficulty:
		...
		3:
			# 专家难度：尝试使用 API
			var api_move = await _get_api_move(board, ai_side)
			if api_move and not api_move.is_empty():
				return api_move
...
func _generate_fen(board: Array, ai_side: int) -> String:
	var fen = ""
	for y in range(ROWS):
		var empty_count = 0
		for x in range(COLS):
			...
			if id == "":
				...
				var side_prefix = id.substr(0, 1)
				var type = id.substr(1, 1)
				var char_code = ""
				
				if side_prefix == "r":
					char_code = type.to_upper()
				else:
					char_code = type.to_lower()
				fen += char_code
	...
	var turn = "b" if ai_side == 2 else "w"
	fen += " " + turn + " - - 0 1"
	return fen

func _get_api_move(board: Array, ai_side: int) -> Dictionary:
	var fen = _generate_fen(board, ai_side)
	var url = "http://www.chessdb.cn/chessdb.php?action=query&board=" + fen.uri_encode() + "&learn=1"
	...
	if error != OK:
		print("HTTP请求失败，错误码: ", error)
		return {}
	
	var result = await http_request.request_completed
	print("API响应结果: ", result)
	print("结果[0] (状态): ", result[0])
	print("结果[1] (HTTP响应码): ", result[1])
	...
	if result[1] != 200:
		print("HTTP响应码不是200，实际为: ", result[1])
		return {}
	
	var body_str = result[3].get_string_from_utf8()
	print("响应体字符串: ", body_str)
	
	if not body_str.begins_with("move:"):
		print("响应不以'move:'开头")
		return {}
	
	var parts = body_str.split(",")
	print("响应分割部分: ", parts)
	var move_part = parts[0]
	print("移动部分: ", move_part)
	var move_str = move_part.substr(5)
	print("移动字符串: ", move_str)
	
	if move_str.length() != 4:
		print("移动字符串长度不是4，实际长度: ", move_str.length())
		return {}
	
	var f_x = move_str.unicode_at(0) - "a".unicode_at(0)
	var f_y_db = int(move_str.substr(1, 1))
	var t_x = move_str.unicode_at(2) - "a".unicode_at(0)
	var t_y_db = int(move_str.substr(3, 1))
	
	print("解析坐标:")
	print("f_x: ", f_x, " (from ", move_str[0], ")")
	print("f_y_db: ", f_y_db, " (数据库格式)")
	print("t_x: ", t_x, " (from ", move_str[2], ")")
	print("t_y_db: ", t_y_db, " (数据库格式)")
	
	# 中国象棋棋盘转换：数据库使用底部为0，你的代码可能顶部为0
	var f_y = 9 - f_y_db  # 假设数据库从下往上，你需要从上往下
	var t_y = 9 - t_y_db
	...
	if f_x < 0 or f_x >= COLS or f_y < 0 or f_y >= ROWS: 
		print("起始坐标越界")
		return {}
	if t_x < 0 or t_x >= COLS or t_y < 0 or t_y >= ROWS: 
		print("目标坐标越界")
		return {}
	
	var from_pos = Vector2i(f_x, f_y)
	var to_pos = Vector2i(t_x, t_y)
	
	var legal_moves = _generate_all_moves(board, ai_side)
	print("生成的合法移动数量: ", legal_moves.size())
	
	# 打印API建议的移动
	print("API建议移动: ", {
		"from": from_pos,
		"to": to_pos,
		"piece": board[f_y][f_x] if f_y >= 0 and f_y < ROWS and f_x >= 0 and f_x < COLS else "N/A",
		"target": board[t_y][t_x] if t_y >= 0 and t_y < ROWS and t_x >= 0 and t_x < COLS else "N/A"
	})
	
	# 打印前几个合法移动进行比较
	for i in range(min(5, legal_moves.size())):
		var m = legal_moves[i]
		print("合法移动 ", i, ": ", m.from, " -> ", m.to)
	
	# 搜索匹配的移动
	for m in legal_moves:
		if m.from == from_pos and m.to == to_pos:
			print("找到匹配的移动!")
			return m

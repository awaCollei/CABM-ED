...
func _get_valid_path(res_path: String) -> String:
	if FileAccess.file_exists(res_path):
		return res_path
	var user_path = "user://" + res_path.get_file()
	if FileAccess.file_exists(user_path):
		return user_path
	return ""

func _load_stop_words():
	var path = _get_valid_path("res://addons/jieba/config/stop_words.utf8")
	_stop_words = {}
	if path != "":

...
func _ready():
	# 资源转移现在由启动器（GameLauncher）控制，不再在此同步阻塞
	# 这样可以避免手机端启动时卡死
		...
		if file:
			...
			if json.parse(json_string) == OK:
				...
				if map_data.has("world") and map_data.world.has("persistent_scenes"):
					...
					print("加载持久化场景列表: ", persistent_scenes)
	
	# 1. 删除opened_chests（刷新宝箱），但保留持久化场景的数据
	if save_data.has("chest_system_data") and save_data.chest_system_data.has("opened_chests"):
		var old_chests = save_data.chest_system_data.opened_chests
		var new_chests = {}
		
		# 保留持久化场景的宝箱数据
		for key in old_chests.keys():
			var scene_id = key.split(":")[0] if ":" in key else ""
			if scene_id in persistent_scenes:
				new_chests[key] = old_chests[key]
				print("保留持久化场景的宝箱: ", key)
		
		save_data.chest_system_data.opened_chests = new_chests
		print("已刷新宝箱（保留 %d 个持久化场景的宝箱）" % new_chests.size())
	
	# 2. 删除掉落物（刷新地图），但保留持久化场景的数据
	if save_data.has("drop_system_data") and save_data.drop_system_data.has("drops_by_scene"):
		var old_drops = save_data.drop_system_data.drops_by_scene
		var new_drops = {}
		
		# 保留持久化场景的掉落物数据
		for scene_id in old_drops.keys():
			if scene_id in persistent_scenes:
				new_drops[scene_id] = old_drops[scene_id]
				print("保留持久化场景的掉落物: ", scene_id)
		
		save_data.drop_system_data.drops_by_scene = new_drops
		print("已刷新掉落物（保留 %d 个持久化场景的掉落物）" % new_drops.size())
	
	# 3. 删除shop_system_data（刷新商店）
	if save_data.has("shop_system_data"):
		save_data.shop_system_data = {}
		print("已删除shop_system_data，商店已刷新")
	
	# 4. 删除enemy_system_data（刷新敌人），但保留持久化场景的数据
	if save_data.has("enemy_system_data"):
		var old_enemies = save_data.enemy_system_data
		var new_enemies = {}
		
		# 保留持久化场景的敌人数据
		for scene_id in old_enemies.keys():
			if scene_id in persistent_scenes:
				new_enemies[scene_id] = old_enemies[scene_id]
				print("保留持久化场景的敌人: ", scene_id)
		
		save_data.enemy_system_data = new_enemies
		print("已刷新敌人（保留 %d 个持久化场景的敌人）" % new_enemies.size())
...
func _add_missing_unique_items_on_daily_refresh():
	...
	inventory_mgr._scan_container_for_unique_items(inventory_mgr.cook_prep_container, "cook_prep", item_counts, item_locations)
	...
	# 扫描持久化场景的宝箱
	inventory_mgr._scan_persistent_chests_for_unique_items(item_counts, item_locations)
	
	# 扫描持久化场景的掉落物
	inventory_mgr._scan_persistent_drops_for_unique_items(item_counts, item_locations)
	
	# 添加缺失的唯一物品到仓库
	for item_id in unique_items:
		if item_counts[item_id] == 0:
			print("[SaveManager] 唯一物品 ", item_id, " 缺失（可能在非持久化场景的箱子中），添加到仓库")
...
# 注意：资源转移功能已移至 ResourceManager
# 如需添加新的初始资源，请编辑 config/resources.json

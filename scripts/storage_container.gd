...
func sort() -> void:
	...
	# 2. 合并相同物品（需满足 ID 且所有其他属性（除 count 外）一致）
	var stacked_items = []
	...
	for item in items:
		...
		if max_stack > 1:
			for existing in stacked_items:
				# 检查 ID 和除了 count 以外的所有属性是否一致
				if _is_item_identical_except_count(item, existing):
					var current_count = int(existing.count)
					if current_count < max_stack:
						var space = max_stack - current_count
						var add_count = min(int(item.count), space)
						existing.count = current_count + add_count
	...
	# 3. 按 item_id 排序
	stacked_items.sort_custom(func(a, b): return a.item_id < b.item_id)

	# 4. 更新存储数组
	storage.clear()
	storage.resize(size)
	for i in range(size):
		if i < stacked_items.size():
			storage[i] = stacked_items[i]
		else:
			storage[i] = null
	
	storage_changed.emit()

func _is_item_identical_except_count(item_a: Dictionary, item_b: Dictionary) -> bool:
	"""检查两个物品除了数量外是否完全相同"""
	if item_a.item_id != item_b.item_id:
		return false
	
	# 比较所有键（排除 count）
	var keys_a = item_a.keys()
	var keys_b = item_b.keys()
	...
	for key in keys_a:
		if key == "count":
			continue
		if not item_b.has(key) or item_a[key] != item_b[key]:
			return false

...
var _current_user_message: String = ""  # 保存当前正在处理的用户消息
...
func _finalize_stream_success() -> void:
	...
	if not _assistant_buffer.strip_edges().is_empty() and not _current_user_message.is_empty():
		...
		conversation_history.append({"role": "user", "content": _current_user_message, "timestamp": ts})
		conversation_history.append({"role": "assistant", "content": _assistant_buffer, "timestamp": ts})
		
		# 可选：打印保存的对话信息
		print("对话已保存 - 用户: %s, 助手: %s..." % [_current_user_message.left(20), _assistant_buffer.left(20)])

...
func show_history():
	...
	# 隐藏对话选项
	if parent_dialog.options_container != null:
		parent_dialog.options_container.visible = false
		print("历史面板：隐藏对话选项")
	...
	# 隐藏顶部输入框（在设置is_history_visible之后）
	if parent_dialog.has_method("_update_top_input_visibility"):
		parent_dialog._update_top_input_visibility(false)
...
func hide_history():
	...
	# 显示对话选项
	if parent_dialog.options_container != null:
		parent_dialog.options_container.visible = true
		print("历史面板：显示对话选项")
	
	# 显示顶部输入框
	if parent_dialog.has_method("_update_top_input_visibility"):
		parent_dialog._update_top_input_visibility(parent_dialog.visible and parent_dialog.top_input_enabled and parent_dialog.is_input_mode)

...
var flip_audio_player: AudioStreamPlayer = null  # 翻炒音效
...
func _ready():
	...
	# 连接锅的点击事件
	if pan_texture:
		pan_texture.gui_input.connect(_on_pan_gui_input)
		pan_texture.mouse_filter = Control.MOUSE_FILTER_STOP
...
func _cleanup_non_ingredient_items():
	...
	for i in range(prep_container.storage.size()):
		...
		if item != null:
			...
			var item_type = item_config.get("type")
			# 允许“食材”和“食品”留在准备区
			if item_type != "食材" and item_type != "食品":
				# 不是允许的类型，移回背包
...
func _try_add_selected_ingredient_to_pan():
	...
	if item_config.get("type") != "食材":
		...
		if not main_scene:
			return false
		MessageDisplay.show_failure_message("这东西不能用于烹饪")
		return
...
func _on_pan_gui_input(event: InputEvent):
	"""锅的点击事件处理"""
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		# 用户指定的点击判定区域：直接使用坐标区间
		var click_area = Rect2(90, 70, 260, 260) 
		if click_area.has_point(event.position):
			if not cook_manager.pan_ingredients.is_empty():
				_flip_pan()
...
func _flip_pan():
	...
	# 播放翻炒音效
	if flip_audio_player:
		flip_audio_player.play()
...
func _add_ingredient_to_pan(item_id: String) -> bool:
	...
	if cook_manager.pan_ingredients.size() >= 20:
		...
		if not main_scene:
			return false
		MessageDisplay.show_failure_message("锅已经满了！")
		return false
	...
	# 创建食材精灵 - 使用刚添加的食材
	var new_ingredient = cook_manager.pan_ingredients[-1]
	_create_ingredient_sprite(new_ingredient)
	return true

func _get_pan_rect() -> Rect2:
	"""获取锅的矩形区域（相对于锅容器）"""
	return Rect2(100,75,140,140)

func _rebuild_ingredient_sprites():
	"""重建所有食材精灵"""
	_clear_ingredient_sprites()
	for ingredient in cook_manager.pan_ingredients:
		_create_ingredient_sprite(ingredient)

func _create_ingredient_sprite(ingredient:CookManager.PanIngredient):
	"""创建食材精灵，使用指定的食材数据"""
	if not pan_texture:
		return
	...
	sprite.pivot_offset = Vector2(60, 60)
	...
	# 使用传入的食材数据设置位置和旋转
	sprite.position = ingredient.position
...
func _on_serve_pressed():
	...
	var has_empty = false
	if prep_container:
		for item in prep_container.storage:
			if item == null:
				has_empty = true
				break
	...
	if not has_empty:
		# 播报信息
		MessageDisplay.show_failure_message("厨房已经放不下东西啦，先整理一下吧")
		return
...
func _create_flip_audio():
	"""创建翻炒音效播放器"""
	flip_audio_player = AudioStreamPlayer.new()
	add_child(flip_audio_player)
	
	# 加载翻炒音效
	var audio_path = "res://assets/audio/effect/flip.mp3"
	if ResourceLoader.exists(audio_path):
		var audio_stream = load(audio_path)
		if audio_stream:
			flip_audio_player.stream = audio_stream
	else:
		print("警告: 翻炒音效文件不存在: ", audio_path)
...
func _record_dish(ingredients: Array):
	...
	var CookManagerClass = load("res://scripts/cook/cook_manager.gd")
	var MealNameGeneratorClass = load("res://scripts/cook/meal_name.gd")
	var generator = MealNameGeneratorClass.new()
	# 将 CookManagerClass 作为参数传递
	var generated_dish_name = generator.generate_meal_name(ingredients, items_config, CookManagerClass)
	# 调用 cook_manager 的 record_dish，传入生成的菜名
	var dish = cook_manager.record_dish(ingredients, items_config, generated_dish_name)

...
func _create_ingredient_sprite(item_id: String):
	...
	if cook_manager.pan_ingredients.size() > 0:
		...
		sprite.position = last_ingredient.position
...
func _show_result(ingredients: Array):
	...
	for ingredient in ingredients:
		...
		# 使用中心点作为位置，而不是左上角
		sprite.position = rand_pos - Vector2(60, 60)  # 居中偏移（食材大小120x120）
		sprite.pivot_offset = Vector2(60, 60)  # 设置旋转中心为食材中心
...
func _get_bowl_rect(bowl_container: Control) -> Rect2:
	...
	var margin = bowl_container.size * 0.35  # 缩小的边距

...
const CHEST_STORAGE_SIZE = 12  # 普通宝箱格子数量
const STORAGE_CHEST_SIZE = 60  # 储物箱格子数量
...
func generate_chest_loot(chest_type: String) -> Array:
	"""生成宝箱战利品"""
	# 储物箱不生成战利品，返回空的储物箱
	if chest_type == "storage_chest":
		var storage = []
		storage.resize(STORAGE_CHEST_SIZE)
		for i in range(STORAGE_CHEST_SIZE):
			storage[i] = null
		return storage

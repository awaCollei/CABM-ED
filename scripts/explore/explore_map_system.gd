...
var current_map_type: String = ""  # 当前地图类型（safe/explore等）

# 地图参数
const MINIMAP_SIZE = Vector2(150, 150)
const MAP_COMPRESSION_RATIO = 16  # 地图图片的压缩比例（8x8压缩为1px）
...
var position_calibration_map: Dictionary = {
	"forest": Vector2(-500, 0), 
	"darkice": Vector2(-730, 0), 
	"SnowfieldBase": Vector2(5, 0)
}
...
func create_minimap_ui() -> Control:
	...
	# 根据地图类型设置边框颜色
	var border_color = _get_minimap_border_color()
...
func _load_map_type():
	...
	var map_config = json.data
	if not map_config.has("world") or not map_config.world.has("points"):
		current_map_type = ""
		return
	
	# 查找当前场景的地图类型
	for point in map_config.world.points:
		if point.get("id") == current_scene_name:
			current_map_type = point.get("type", "")
			return
	
	current_map_type = ""

func _get_minimap_border_color() -> Color:
	"""根据当前地图类型获取小地图边框颜色"""
	if current_map_type == "safe":
		return Color(0.3, 0.9, 0.3, 0.8)  # 绿色边框 - 安全区域
	elif current_map_type == "explore":
		return Color(0.5, 0.5, 0.5, 0.5)  # 灰色边框 - 探索区域
	else:
		return Color(0.5, 0.5, 0.5, 0.5)  # 默认灰色边框

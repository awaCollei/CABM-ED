...
var auto_aim_range: float = 600.0
var current_auto_aim_target: Node2D = null
...
func _process(delta):
	...
	if player and scene_state and scene_state.is_active():
		...
		# 移动端自动瞄准：持续朝向当前锁定目标
		if player.is_mobile and auto_aim_enabled:
			_ensure_auto_aim_target()
			if current_auto_aim_target and player.has_method("set_aim_direction"):
				var dir = (current_auto_aim_target.global_position - player.global_position)
				if dir.length() > 0.01:
					player.set_aim_direction(dir.normalized())
...
func _open_map_from_explore():
	...
	if memory_saver:
		...
		if has_node("/root/SaveManager"):
			...
			character_name = smem.get_character_name()

		# 检查当前场景是否是安全区
		var is_safe_zone = _is_safe_zone(scene_state.current_explore_id)
		...
		if is_safe_zone:
			base_text = "我和%s去了一趟%s。" % [user_name, scene_name]
			tail_text = ""
		else:
			base_text = "我和%s在%s进行了探索，" % [user_name, scene_name]
			tail_text = "我们顺利撤离。"
		...
		if not conversation_history.is_empty() and explore_chat_summary_manager:
			...
			var archived_summary = await explore_chat_summary_manager.call_explore_summary_api(
				conversation_history, user_name, scene_name, character_name)
...
func _handle_death_complete_save():
	...
	if memory_saver:
		...
		if is_safe_zone:
			# 安全区：简化记忆文本（死亡情况下也使用简化格式）
			base_text = "我和%s去了一趟%s。" % [user_name, scene_name]
			tail_text = ""  # 安全区没有tail_text
		else:
			# 非安全区：完整记忆文本
			base_text = "我和%s在%s进行了探索，" % [user_name, scene_name]
			tail_text = "我们在战斗中倒下了。"
...
func _load_auto_aim_state():
	"""加载自动瞄准开关（默认开启）"""
	auto_aim_enabled = true
	if SaveManager and SaveManager.save_data.has("explore_auto_aim_enabled"):
		auto_aim_enabled = bool(SaveManager.save_data.explore_auto_aim_enabled)

func _save_auto_aim_state():
	"""保存自动瞄准开关"""
	if not SaveManager:
		return
	SaveManager.save_data.explore_auto_aim_enabled = auto_aim_enabled
	SaveManager.save_game(SaveManager.current_slot)

func _ensure_auto_aim_target():
	"""确保当前有有效的自动瞄准目标"""
	if not enemy_manager or not player:
		current_auto_aim_target = null
		return

	# 当前目标失效则清空
	if current_auto_aim_target:
		if (not is_instance_valid(current_auto_aim_target)) \
			or (player.global_position.distance_to(current_auto_aim_target.global_position) > auto_aim_range) \
			or (("health" in current_auto_aim_target) and current_auto_aim_target.health <= 0):
			current_auto_aim_target = null

	# 没有目标时，选择最近的敌人
	if current_auto_aim_target == null:
		current_auto_aim_target = _find_nearest_enemy_in_range()
...
func _find_nearest_enemy_in_range() -> Node2D:
	...
	for enemy in enemy_manager.active_enemies:
		...
		if dist <= auto_aim_range and dist < best_dist:
			best_dist = dist
			best = enemy
	return best

func _collect_auto_aim_targets() -> Array:
	"""收集范围内可用于自动瞄准的敌人（按距离排序）"""
	var result: Array = []
	if not enemy_manager or not player:
		return result
	for enemy in enemy_manager.active_enemies:
		...
		if dist <= auto_aim_range:
			result.append({"enemy": enemy, "dist": dist})
	# 使用自定义排序（手动冒泡，数量通常很少）
	for i in range(result.size()):
		for j in range(i + 1, result.size()):
			if not _sort_auto_aim_target(result[i], result[j]):
				var tmp = result[i]
				result[i] = result[j]
				result[j] = tmp
	return result

func _sort_auto_aim_target(a: Dictionary, b: Dictionary) -> bool:
	return a["dist"] < b["dist"]

func _select_next_auto_aim_target():
	"""切换到下一个自动瞄准目标"""
	var targets = _collect_auto_aim_targets()
	if targets.is_empty():
		current_auto_aim_target = null
		return
	...
	for i in range(targets.size()):
		if targets[i]["enemy"] == current_auto_aim_target:
			index = i
			break

	if index == -1:
		# 当前目标不在列表中，从最近的开始
		current_auto_aim_target = targets[0]["enemy"]
	else:
		index = (index + 1) % targets.size()
		current_auto_aim_target = targets[index]["enemy"]

	# 立即将朝向对准新目标（仅移动端自动瞄准生效时）
	if player and player.is_mobile and auto_aim_enabled and current_auto_aim_target and player.has_method("set_aim_direction"):
		var dir = (current_auto_aim_target.global_position - player.global_position)
		if dir.length() > 0.01:
			player.set_aim_direction(dir.normalized())

func _on_mobile_auto_aim_toggled(enabled: bool):
	"""移动端自动瞄准按钮切换"""
	auto_aim_enabled = enabled
	if player and player.has_method("set_auto_aim_active"):
		player.set_auto_aim_active(auto_aim_enabled)
	if not auto_aim_enabled:
		current_auto_aim_target = null
	else:
		# 开启时立即锁定一个目标并朝向它
		_ensure_auto_aim_target()
		if current_auto_aim_target and player and player.is_mobile and player.has_method("set_aim_direction"):
			var dir = (current_auto_aim_target.global_position - player.global_position)
			if dir.length() > 0.01:
				player.set_aim_direction(dir.normalized())
	_save_auto_aim_state()

func _on_mobile_switch_target_pressed():
	"""移动端切换目标按钮点击"""
	if not auto_aim_enabled or not player or not player.is_mobile:
		return
	_select_next_auto_aim_target()
...
func _create_mobile_ui():
	...
	if ResourceLoader.exists("res://scenes/mobile_ui.tscn"):
		...
		if mobile_ui:
			...
			# 自动瞄准 UI 初始化与信号
			if mobile_ui.has_method("set_auto_aim_enabled"):
				_load_auto_aim_state()
				mobile_ui.set_auto_aim_enabled(auto_aim_enabled)
				if player and player.has_method("set_auto_aim_active"):
					player.set_auto_aim_active(auto_aim_enabled)
			if mobile_ui.has_signal("auto_aim_toggled"):
				mobile_ui.auto_aim_toggled.connect(_on_mobile_auto_aim_toggled)
			if mobile_ui.has_signal("switch_target_pressed"):
				mobile_ui.switch_target_pressed.connect(_on_mobile_switch_target_pressed)
...
func _is_safe_zone(explore_id: String) -> bool:
	"""检查指定的探索场景是否是安全区"""
	...
	for point in map_data.world.points:
		if point.get("id", "") == explore_id:
			return point.get("type", "") == "safe"

...
func _should_perform_kg_search() -> bool:
	...
	if ai_service and ai_service.config_loader:
		# 尝试从全局配置获取内存配置
		var memory_config = ai_service.config_loader.config.get("knowledge_memory", {})
...
func _retrieve_knowledge_memory(query: String) -> String:
	...
	if ai_service and ai_service.config_loader:
		knowledge_config = ai_service.config_loader.config.get("knowledge_memory", {})
	...
	var mem_prompts = knowledge_config.get("prompts", {})

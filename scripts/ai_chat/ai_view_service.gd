...
func describe_image(path: String) -> String:
	...
	if ai == null or not ai.config_loader:
		description_error.emit("AIService 不可用")
		return ""
	var cfg = ai.config_loader.get_model_config("view_model")
	var model = cfg.get("model", "")
	var base_url = cfg.get("base_url", "")
	var api_key = cfg.get("api_key", "")
	...
	if model.is_empty() or base_url.is_empty():
		...
		return ""
	if api_key.is_empty():
		description_error.emit("视图模型 API 密钥未配置")
		return ""

...
func _get_effective_api_key_global() -> String:
	...
	# 未启用内置时：用对话槽位的密钥作为「全局」判断（与 start_chat 一致）
	return _get_effective_api_key("chat_model")

func _get_effective_api_key(slot_name: String) -> String:
	"""获取某配置槽位的有效 API 密钥。slot_name 为 chat_model、embedding_model 等。
	user:// 根级 api_key 仅供展示，不参与请求；请求只使用各槽位下的 api_key。"""
	if slot_name.is_empty():
		return ""

	var model_id = ""
	if config.has(slot_name) and config[slot_name].has("model"):
		model_id = str(config[slot_name]["model"])
	...
	if config_mgr and config_mgr.has_method("load_use_builtin_key"):
		...
		if use_builtin:
			# 检查模型是否允许使用内置密钥
			if not model_id.is_empty() and builtin_key_manager:
				if not builtin_key_manager.is_model_allowed(model_id):
					push_error("模型 " + model_id + " 不支持使用内置密钥")
					return "INVALID_MODEL_FOR_BUILTIN_KEY"
			# 返回内置密钥
			if builtin_key_manager:
				var builtin_key = builtin_key_manager.get_builtin_key()
	...
	var slot_cfg = config.get(slot_name, {})
	if typeof(slot_cfg) != TYPE_DICTIONARY:
		return ""
	var key: String = str(slot_cfg.get("api_key", ""))
	if not key.is_empty():
		return key
	if slot_name != "chat_model":
		var chat_cfg = config.get("chat_model", {})
		if typeof(chat_cfg) == TYPE_DICTIONARY:
			var chat_key: String = str(chat_cfg.get("api_key", ""))
			if not chat_key.is_empty():
				return chat_key
	return ""
...
func get_model_config(model_name: String) -> Dictionary:
	...
	# 如果配置中有 api_key，按槽位解析有效密钥（内置或该槽位/回退到 chat）
	if model_config.has("api_key"):
		model_config["api_key"] = _get_effective_api_key(model_name)
	return model_config
...
func _inject_effective_keys():
	...
	for slot_name in model_names:
		if config.has(slot_name) and config[slot_name].has("model"):
			var effective_key = _get_effective_api_key(slot_name)
			if not effective_key.is_empty():
				print("注入有效密钥: ", effective_key.substr(0, 5)+"...")
				config[slot_name]["api_key"] = effective_key

...
func _ready():
	...
	# 选项生成器
	options_generator = preload("res://scripts/ai_chat/ai_options_generator.gd").new()
	add_child(options_generator)
	options_generator.owner_service = self
...
func transcribe_audio(audio_bytes: PackedByteArray, filename: String = "recording.wav"):
	...
	var stt_cfg = config_loader.get_model_config("stt_model")
	var model = stt_cfg.get("model", "")
	var base_url = stt_cfg.get("base_url", "")
	var timeout_s = int(stt_cfg.get("timeout", 10))
	var api_key = stt_cfg.get("api_key", "")

	if api_key.is_empty():
		stt_error.emit("STT 模型 API 密钥未配置")
		return
	if model.is_empty():
...
func start_chat(user_message: String = "", trigger_mode: String = "passive", item_data: Dictionary = {}):
	"""开始对话
	...
	if config_loader.api_key.is_empty():
		chat_error.emit("API密钥未配置，可以在配置选项中开启内置密钥")
		MessageDisplay.show_failure_message("群公告都不看玩nm    ——狼尊")
		return
	...
	# 如果有物品数据，设置到PromptBuilder的meta中
	if not item_data.is_empty():
		prompt_builder.set_meta("pending_item", item_data)
		...
		# 清除之前的物品数据
		if prompt_builder.has_meta("pending_item"):
			prompt_builder.remove_meta("pending_item")
...
func _finalize_stream_response():
	...
	if not item_data.is_empty():
		_handle_item_response(extracted_fields, item_data)
...
func _on_request_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray):
	...
	print("请求完成回调 - 类型: %s, 结果: %d, 响应码: %d" % [request_type, result, response_code])
	...
	if response_code != 200:
		...
		# 根据请求类型分别处理错误
		if request_type == "summary":
			print("总结请求API错误，触发重试机制")
			_handle_summary_failure(error_msg)
		elif request_type == "relationship":
			print("关系模型API错误: " + error_msg)
			# 关系模型失败不影响主流程，只记录错误
		else:
			chat_error.emit(error_msg)
		return
	...
	if json.parse(response_text) != OK:
		...
		# 根据请求类型分别处理错误
		if request_type == "summary":
			print("总结响应解析失败，触发重试机制")
			_handle_summary_failure(error_msg)
		elif request_type == "relationship":
			print("关系模型响应解析失败: " + error_msg)
		return
...
func _handle_summary_failure(error_msg: String):
	...
	if summary_retry_count <= max_summary_retries:
		...
		MessageDisplay.show_failure_message("获取总结失败，已达到最大重试次数（%d次），将丢弃此记忆条目" % max_summary_retries)
...
func _on_options_generated(options: Array):
	"""选项生成完成回调"""
	print("选项生成完成: ", options)
	options_generated.emit(options)

func _on_options_error(error_message: String):
	"""选项生成错误回调"""
	print("选项生成失败: ", error_message)

func _handle_item_response(extracted_fields: Dictionary, item_data: Dictionary):
	"""处理物品接收响应
	...
	var item_action = int(extracted_fields.item)
	var inventory_mgr = get_node_or_null("/root/InventoryManager")
	...
	# 获取物品名称用于显示
	var item_config = inventory_mgr.get_item_config(item_data.item_id)
	var item_name = item_config.get("name", "物品")
	...
	match item_action:
		...
		2:  # 立即使用（消耗一个）
			print("雪狐立即使用了物品: ", item_data.item_id)
		...
		3:  # 使用全部
			print("雪狐使用了全部物品: ", item_data.item_id)
			_show_item_feedback("%s使用了全部%s" % [character_name, item_name])
			# TODO: 应用物品效果（如果需要）
	...
	var prompt_builder = get_node("/root/PromptBuilder")
	if prompt_builder and prompt_builder.has_meta("pending_item"):
		prompt_builder.remove_meta("pending_item")
...
func _clear_uploaded_item():
	...
	# 获取物品输入模块
	var item_input = chat_dialog.get_node_or_null("ItemInput")
	if not item_input:
		return
	
	# 清除选中的物品（不放回背包，因为已经处理过了）
	if item_input.has_method("has_selected_item") and item_input.has_selected_item():
		item_input.selected_item_data.clear()
		if item_input.has_method("_update_button_icon"):
			item_input._update_button_icon()
		print("已清除上传的物品状态")

func _show_item_feedback(message: String):
	"""显示物品处理反馈"""
	var main_scene = get_tree().current_scene
	if not main_scene:
		return
	
	MessageDisplay.show_info_message(message)

...
func log_api_request(log_type: String, body: Dictionary, json_body: String):
	...
	if typeof(body) == TYPE_DICTIONARY:
		...
		model_val = body.model if body and body.has("model") else ""
	...
	if typeof(body) == TYPE_DICTIONARY:
		...
		temperature_val = body.get("temperature", "")
		top_p_val = body.get("top_p", "")
		var msgs = body.get("messages", [])
		if typeof(msgs) == TYPE_ARRAY:
			messages_count = msgs.size()
	else:
		# try property access for non-dict types
		if body and body.has("max_tokens"):
			max_tokens_val = body.max_tokens
		if body and body.has("temperature"):
			temperature_val = body.temperature
		if body and body.has("top_p"):
			top_p_val = body.top_p
		if body and body.has("messages") and typeof(body.messages) == TYPE_ARRAY:
			messages_count = body.messages.size()

	log_file.store_line("  model: " + str(model_val))
	log_file.store_line("  max_tokens: " + str(max_tokens_val) + " (类型: " + type_string(typeof(max_tokens_val)) + ")")
	log_file.store_line("  temperature: " + str(temperature_val) + " (类型: " + type_string(typeof(temperature_val)) + ")")
	log_file.store_line("  top_p: " + str(top_p_val) + " (类型: " + type_string(typeof(top_p_val)) + ")")
	log_file.store_line("  messages 数量: " + str(messages_count))
	log_file.store_line("=".repeat(50))
	log_file.close()

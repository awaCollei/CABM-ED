...
var static_offers_cache: Dictionary = {} # 缓存静态商店商品
...
func _ready():
	...
	# 预加载静态商店数据（可选，也可以懒加载）
	# _preload_static_shops()

func _preload_static_shops():
	var categories = ["produce", "grocery", "snacks", "drinks"]
	for cat in categories:
		get_static_offers(cat)
...
func get_static_offers(category: String) -> Array:
	if static_offers_cache.has(category):
		return static_offers_cache[category]
	
	var path = "res://config/shop/%s.json" % category
	if not FileAccess.file_exists(path):
		# print("警告: 静态商店配置不存在: ", path)
		return []
		...
		if typeof(data) == TYPE_ARRAY:
			for item in data:
				var offer = {
					...
					"requires": [{"item_id": "credit", "count": int(item.price)}],
					"gives": [{"item_id": item.item_id, "count": 1}],
					"limit": -1, # 无限
					"bought": 0,
					"is_static": true
				}
	...
	static_offers_cache[category] = offers
	return offers
...
func trade_offer(offer_id: String) -> bool:
	...
	# 如果没找到，查找静态缓存
	if offer == null:
		for cat in static_offers_cache:
			for o in static_offers_cache[cat]:
				if o.id == offer_id:
					offer = o
					break
	...
	if offer.get("limit", 1) != -1 and int(offer.bought) >= int(offer.limit):
...
func _save_to_save():
	...
	if not save_mgr.save_data.has("shop_system_data"):
		save_mgr.save_data.shop_system_data = {"active_offers": []}

...
var max_history_count: int = 12  # 故事模式携带12条上下文
...
func _build_system_prompt(story_data: Dictionary, _current_node_id: String, story_context: Dictionary) -> String:
	...
	# 如果有自定义身份，使用自定义身份
	if not character_role.is_empty() or not user_role.is_empty():
		if not character_role.is_empty():
			base_prompt += "你的身份：" + character_role + "\n"
...
func _finalize_streaming_response():
	...
	if not streaming_full_reply.is_empty():
		if not conversation_history.is_empty() and conversation_history.back().role == "assistant":
			# 如果最后一条是助手消息，则合并内容（用于继续生成）
			conversation_history.back().content += streaming_full_reply
		else:
			# 否则追加新消息

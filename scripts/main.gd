...
@onready var right_click_area = $RightClickArea
@onready var debug_helper = $CharacterDebugHelper
@onready var audio_manager = $AudioManager
@onready var interactive_elements_container = $InteractiveElements
...
func _setup_managers():
	...
	# 初始化消息显示管理器 (使用全局单例 MessageDisplay)
	MessageDisplay.initialize(scene_manager)
	...
	costume_manager.initialize(character)
...
func _create_dynamic_elements():
	...
	for element_id in interactive_elements:
		# 检查容器中是否已存在该元素
		var element_path = "InteractiveElements/" + element_id
		if has_node(element_path):
			dynamic_elements[element_id] = get_node(element_path)
			...
			if element_scene:
				...
				interactive_elements_container.add_child(element)
...
func _on_character_clicked(char_position: Vector2, char_size: Vector2):
	...
	if has_node("/root/EventManager"):
		...
		if not result.success:
			if result.message != "":
				MessageDisplay.show_failure_message(result.message)
			return
...
func _on_shop_action_triggered(_action: String):
	...
	if not ResourceLoader.exists(shop_scene_path):
		MessageDisplay.show_failure_message("错误：商店界面未找到: " + shop_scene_path)
		return
	...
	if shop_scene:
		...
		MessageDisplay.show_failure_message("错误：无法实例化商店界面")
...
func _on_farming_action_triggered(_action: String):
	...
	if not ResourceLoader.exists(farming_scene_path):
		MessageDisplay.show_failure_message("错误：菜园界面未找到: " + farming_scene_path)
		return
	...
	if farming_scene:
		...
		MessageDisplay.show_failure_message("错误：无法实例化菜园界面")
...
func _on_companion_mode_selected():
	...
	if scene_manager.current_scene != "studyroom":
		MessageDisplay.show_failure_message("只能在书房进入陪伴模式")
		return
	...
	if has_node("/root/SceneTransition"):
		var transition = get_node("/root/SceneTransition")
		await transition.change_scene_with_fade("res://scenes/companion_mode.tscn")
	else:
		# 如果没有过渡管理器，直接切换
		get_tree().change_scene_to_file("res://scenes/companion_mode.tscn")

func _on_sleep_mode_selected():
	"""处理助眠模式选择"""
	print("进入助眠模式")
	
	# 验证当前场景是否为卧室
	if scene_manager.current_scene != "bedroom":
		MessageDisplay.show_failure_message("只能在卧室进入助眠模式")
		return
	...
	if has_node("/root/SceneTransition"):
		var transition = get_node("/root/SceneTransition")
		await transition.change_scene_with_fade("res://scenes/sleep_mode.tscn")
	else:
		# 如果没有过渡管理器，直接切换
...
func _handle_timeout_to_input():
	...
	MessageDisplay.show_failure_message(character_name + "默默离开了")
...
func _move_character_to_current_scene():
	...
	if has_node("/root/SaveManager"):
		...
		# 检查是否启用呼唤触发对话
		var should_trigger_dialog = true
		if has_node("/root/AIConfigManager"):
			var config_mgr = get_node("/root/AIConfigManager")
			should_trigger_dialog = config_mgr.load_call_trigger_dialog()
